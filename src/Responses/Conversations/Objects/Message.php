 ...
 *
 * @phpstan-type MessageType array{content: array<int, InputTextType|InputImageType|InputFileType|OutputTextType|RefusalType>, id: string, role: 'unknown'|'user'|'assistant'|'system'|'critic'|'discriminator'|'developer'|'tool', status: 'in_progress'|'completed'|'incomplete', type: 'message'}
 *
...
final class Message implements ResponseContract
{
    ...
    /**
     * @param  array<int, InputText|InputImage|InputFile|OutputText|Refusal>  $content
    ...
    {
        $content = array_map(
            fn (array $item): InputText|InputImage|InputFile|OutputText|Refusal => match ($item['type']) {
    ...
    public function toArray(): array
    {
        return [
            'content' => array_map(
                fn (InputText|InputImage|InputFile|OutputText|Refusal $item): array => $item->toArray(),
                $this->content,
            ),
            'id' => $this->id,
            'role' => $this->role,
